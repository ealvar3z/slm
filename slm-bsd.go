...
// slm_bsd.go
// slm: CLI for OpenAI GPT on OpenBSD/FreeBSD
// On OpenBSD, uses pledge() to restrict syscalls.
// On FreeBSD, no-op (pledge not supported).
package main
...
// run pledge on OpenBSD
func init() {
	if runtime.GOOS == "openbsd" {
		// stdio, read/write config, network
		err := pledge.Pledge("stdio rpath wpath cpath inet dns", "")
		if err != nil {
			log.Fatalf("[PLEDGE] failed: %v", err)
		}
		// no exec promises needed
	}
}
